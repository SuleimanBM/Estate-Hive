...
export async function createApplication(tenantId, data) {
...
export async function getApplicationsForManager(managerId) {
...
export async function approveApplication(applicationId, managerId, opts) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (!managerOwns)
            throw new Error("Forbidden: you don't manage this property");
...
export async function denyApplication(applicationId, managerId, reason) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (!app)
            throw new Error("Application not found");
        if (app.property.managerId !== managerId)
            throw new Error("Forbidden: you don't manage this property");
        await tx.application.update({ where: { id: applicationId }, data: { status: "DENIED" } });
        if (app.tenant.email)
            notifyTenantApplicationUpdated(app.tenant.email, app.id, "DENIED").catch(console.error);
        return { applicationId };
    });
}
// Tenant withdraw
export async function withdrawApplication(applicationId, tenantId) {
